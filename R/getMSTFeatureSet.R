...
getMSTFeatureSet2 = function(x, normalize = FALSE) {
  ...
  res = list(
    "mst_depth_norm_var" = statistics.on.mst.depth$mst_depth_norm_var,
    ...
    "mst_dists_norm_var" = statistics.on.mst.dists$mst_dists_norm_var,
  ...
  res$mst_dists_norm_sum_ratio = normalizeFeature(sum(span_tree_dists) / sum(d), 1 / (n.cities - 1), 2 * pi / (n.cities ** 2 * 1.27))
  return(res)
}
