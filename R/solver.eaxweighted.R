...
#'   Default is \code{FALSE}.
#' @param fitness.type [\code{character(1)}]\cr
#'   Type of fitness function, either \dQuote{classic} or \dQuote{weighted}.
#'   Default is
...
#' @export
run.eaxw = function(solver, instance,
  max.trials = 1L,
  pop.size = 100L,
  off.size = 30L,
  cutoff.time = 10L,
  ...
  fitness.mapping = list("classic" = 0, "weighted" = 1)
  ...
  args = list(max.trials, file.output, pop.size, off.size,
    ...
    snapshot.step, as.integer(log.trajectory), as.integer(with.GPX), as.integer(fitness.mapping[[fitness.type]]))
